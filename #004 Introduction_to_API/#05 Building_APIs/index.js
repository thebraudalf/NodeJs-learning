...
app.get("/jokes/:id", (req, res) => {
  ...
  const getSpecificJoke = jokes.find((joke) => joke.id === id);

  res.json(getSpecificJoke);
});

//3. GET a jokes by filtering on the joke type
app.get("/filter", (req, res) => {
const type = req.query.type;
const getFilteredJoke = jokes.filter((joke) => joke.jokeType === type);

res.json(getFilteredJoke);
});
...
app.post("/jokes", (req, res) => {
    ...
    const sendaJoke = { id: jokes.length + 1, jokeText: text, jokeType: type, };
    jokes.push(sendaJoke);
...
app.patch("/jokes/:id", (req, res) => {
 ...
 const searchIndex = jokes.findIndex((joke) => joke.id === id); 
 jokes[searchIndex] = replaceTextType;

 console.log(jokes[searchIndex]);
 res.json(replaceTextType);
});
...
app.delete("/jokes/:id", (req, res) => {
  ...
  if (searchIndex > -1) {
    ...
    res.status(404).json({ message: `The Joke id ${id} is not found.` });
  }
});

//8. DELETE All jokes
app.delete("/all", (req, res) => {
  const IsUserAuthorized = req.query.masterKey;

  if (IsUserAuthorized === masterKey) {
    jokes.splice(0, jokes.length);
    console.log(jokes);
    res.json({ message: "All the Jokes have been deleted." }).sendStatus(200);
  } else {
